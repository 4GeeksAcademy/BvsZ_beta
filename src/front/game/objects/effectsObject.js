...
export class EffectsObjects {
  ...
  resetEmitters() {
    // Destruir emisores existentes antes de asignar null
    if (this.scene.bloodEmitter) {
      this.scene.bloodEmitter.destroy();
    }
    if (this.scene.sparkEmitter) {
      this.scene.sparkEmitter.destroy();
    }
    if (this.scene.explosionFireEmitter) {
      this.scene.explosionFireEmitter.destroy();
    }
    if (this.scene.explosionEmitter) {
      this.scene.explosionEmitter.destroy();
    }

    // Asignar null después de destruir
  ...
  handleZombieHit(bullet, zombie) {
    ...
    const damageText = this.scene.add
      .text(zombie.x + 20, zombie.y - 20, `-${damage}`, {
        font: "23px Arial",
        fill: "#ff0000",
        stroke: "#fff",
        strokeThickness: 3,
  ...
  explosionFireEmitter(target, x = 0, y = 0) {
    // Efecto de sangre al colisionar, recibe un target y desfasamiento de posición del efecto
    if (!this.scene.explosionFireEmitter) {
      this.scene.explosionFireEmitter = this.scene.add.particles(
        0,
        0,
        "explosion_fire",
        {
